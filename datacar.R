...
dmat_train_sev <- xgb.DMatrix(features_train_sev, info = list(label = label_train_sev
                                                              , weight = pre.train_severity$numclaims))
...
#Need to multiply with exposure in prediction since we feed in exposure as an offset
#in base margin, so when predict without basemargin, the prediction will be count/ee
